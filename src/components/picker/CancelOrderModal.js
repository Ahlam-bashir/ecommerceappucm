...
const CancelOrderModal=(props)=>{
  ...
  useEffect(async()=>{
    ...
    .then(response => {
        setUser(response)
        if(orderId!==''||itemId!==''){
            const url=itemId&&orderId?API_URL+'CancelOrder/'+orderId+'?ItemId='+itemId:
            API_URL + 'CancelOrder/'+orderId+'?ItemId='
         ...
         fetch(url, {
           ...
           //  setVisible(false)
             ...
             if(responseJson.statusCode==0){
                 setReasons(responseJson.data.CancelReasons)
  ...
  const cancelOrder=async()=>{
    if(cancelReasonId==0){
      RNToasty.Warn({
        title:'choose reason for cancelling the order',
        position:'center'
      },2000)
      return
    }
    ...
    const datatoSend=
     ...
     await AsyncStorage.getItem('user')
      .then(value => JSON.parse(value))
      .then(response => {
          const url=itemId==''?API_URL + 'CancelOrder/'+orderId+'?ItemId=':
          API_URL + 'CancelOrder/'+orderId+'?ItemId='+itemId
          console.log(url)
        ...
        >
          ...
          >        
              <View style={styles.bottomNavigationView}>
                  <View>
                      <Text type='subheading'>Select the reason for cancelling the order</Text>
                  </View>
                  <FlatList
                  data={reason}
            ...
            <Text type='subheading'>Cancel description </Text>
            ...
            <TouchableOpacity style={{height:40,width:'100%',backgroundColor:Colors.colors.primary,alignItems:'center',justifyContent:'center',margin:8,borderRadius:8}}
             onPress={()=>cancelOrder()}
             >
            <Text type='caption' style={{color:Colors.colors.white}}>Cancel</Text>
            </TouchableOpacity>
              </View>
              </KeyboardAvoidingView>
